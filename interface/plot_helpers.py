...
def get_mrs_info(data):
    ...
    if hasattr(f, "metadata") and f.metadata is not None and "ave_per_rep" in f.metadata:
        info += f"\nNumber of shots per average: {f.metadata['ave_per_rep']}"
    info += f"\nNumber of averages: {len(data)}"
    if hasattr(f, "metadata") and f.metadata is not None and "ave_per_rep" in f.metadata and f.metadata["ave_per_rep"] > 1:
        info += f"→ {int(len(data) / f.metadata['ave_per_rep'])}"
    info += "\n"
